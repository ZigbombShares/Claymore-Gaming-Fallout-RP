if !Prometheus then
	...
	Prometheus.LuaVersion = "1.7.2.00" -- This is different from the website version, they don't have to be same

	Prometheus.LastActionUpdate = "1000-01-01 00:00:00"

	if !file.Exists("prometheus", "DATA") then
		file.CreateDir("prometheus")
	end
...
hook.Add("PostPlayerLoadout", "PrometheusPostPlayerLoadoutHook_NutScript", function(Ply)
	GiveWeapons(Ply)
end)
...
concommand.Add("PrometheusLogToFile", function(Ply)
	if IsValid(Ply) then return end

	Prometheus.LogToFile = true

	MsgC(Color(255,255,0), "\nPrometheus logging to file enabled until next map change or reboot.\n\n")
end)