--[[ Config ]]--
	...
	Prometheus.WebsiteEnabled = false -- If true, the chat command will be enabled (Opens menu)
	...
	Prometheus.Access.AdminMenu = {"founder", "devoloper"} -- Groups that can access the admin menu. Examples: {"admin"}  {"admin", "superadmin", "owner"}     For AssMod use the number of the rank like: {1, 2}

	Prometheus.NotifyEveryone = false -- If true, it will send a notification to everyone that a person on server has gotten a package

	Prometheus.RefreshTime = 60 -- How often should it check for new actions (In seconds)
	...
	Prometheus.Mysql.Host = "192.185.16.74"
	Prometheus.Mysql.Port = 3306
	Prometheus.Mysql.Username = "claymore_db"
	Prometheus.Mysql.Password = "bG#B2m&ucYpNq8n("
	Prometheus.Mysql.DBName = "claymore_fallout_store"
